...
from diffusers.schedulers import FlowMatchEulerDiscreteScheduler, FlowMatchHeunDiscreteScheduler, DPMSolverMultistepScheduler
...
def scale_noise(
    ...
    if sample.device.type == "mps" and torch.is_floating_point(timestep):
        ...
        schedule_timesteps = scheduler.timesteps.to(sample.device, dtype=torch.float32)
    ...
    if scheduler.begin_index is None:
        ...
        step_indices = [scheduler.step_index] * timestep.shape[0]
    else:
        # add noise is called before first denoising step to create initial latent(img2img)
        step_indices = [scheduler.begin_index] * timestep.shape[0]
...
class FluxKontextInpaintPipeline(
    ...
    def prepare_latents(
        ...
        if latents is None:
            ...
            if isinstance(self.scheduler, DPMSolverMultistepScheduler):
                latents = scale_noise(self.scheduler, image_latents, timestep, noise)
            else:   
    ...
    def __call__(
        ...
        # 4. Prepare timesteps
        if isinstance(self.scheduler, FlowMatchEulerDiscreteScheduler) or isinstance(self.scheduler, FlowMatchHeunDiscreteScheduler):
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i < len(timesteps) - 1:
                    ...
                    if isinstance(self.scheduler, DPMSolverMultistepScheduler):
                        init_latents_proper = scale_noise(self.scheduler, init_latents_proper, torch.tensor([noise_timestep]), noise)
                    else:
